...
// calendar. Taking the first name that merely EXISTS picks the empty one and the video
// reads as undated, so walk the names in preference order and take the first that holds a
// real value, falling back to any date property that has one.
function firstDate(props, names) {
  for (const n of names) { const v = dateStart(byName(props, [n])); if (v) return v; }
  for (const k in props) { const v = dateStart(props[k]); if (v) return v; }
  return null;
}
