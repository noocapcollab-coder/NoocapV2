...
// Boards differ in which date column they use, and several carry more than one — Chris's
// board has an empty DUE DATE sitting alongside the POST DATE that actually drives his
// calendar. Taking the first name that merely EXISTS picks the empty one and the video reads
// as undated, so walk the names in preference order and take the first holding a real value.
// Only these names are considered: falling back to "any date property with something in it"
// picks up unrelated columns (follow-ups, review dates) and files videos into wrong weeks.
function firstDate(props, names) {
  for (const n of names) { const v = dateStart(byName(props, [n])); if (v) return v; }
  return null;
}
...
function normalize(row, creator) {
  ...
  // Long videos are always marked "LONG FROM" for the long-form view, so treat that as the
  // only long signal and everything else (SHORT FORM, blank) as short — that way every video
  // gets a tag and nothing genuinely long slips through unmarked.
  const format = /long/i.test(fmtRaw || "") ? "Long Form" : "Short Form";
  const due = firstDate(p, ["POST DATE", "DUE DATE", "Date"]);
