...
function normalize(row, creator) {
  ...
  // Mirror the board's Format field: long when it says long, short when it's set to anything
  // else (Other, reel, short form), and null when the field is blank so nothing is mislabelled.
  const format = /long/i.test(fmtRaw || "") ? "Long Form" : (fmtRaw && fmtRaw.trim()) ? "Short Form" : null;
